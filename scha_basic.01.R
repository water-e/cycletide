...
12.4206012*2,
12.4206012*2/3.,
12.4206012*2/5.) #0.2028035475

names(hours_per_cycle) <- c("LOW","K1","M2","O1","S2","Q1","N2","L2","M4","MK3","M6","MK5","M1","M3","M5")
...
fname <- "F:/projects/stochastic_cycle/freeport_stage.csv"
...
freq_names <- c("LOW","K1","M2","O1","S2","Q1","N2","L2","M4","MK3","M6","MK5","M1","M3","M5")
names(hours_per_cycle)  <- freq_names
radians_per_hour <- 2.*pi/hours_per_cycle
freq_used <- 0.25*radians_per_hour[freq_names]


regress_names <- c("K1","M2","O1","S2","Q1","N2","L2","M4","MK3","M6","MK5")
...
#W[ordertrend+ordercycle*2-1,ordertrend+ordercycle*2-1] <-0.001  # 0.005 worked well
#W[ordertrend+8,ordertrend+8] <- 0.001
...
#select <- 19000:24000
#select <- 16000:24000
plot.ts(ysave[select]*1,ylim=c(-4,4.),col="black")
lines(ylm,col="purple")
...
sumf <- modSmooth$s[select,D0] + modSmooth$s[select,D1] + modSmooth$s[select,D2] + modSmooth$s[select,D3] +modSmooth$s[select,D4] +modSmooth$s[select,D5]
yreconstruct <- sumf + ylm


lines(yreconstruct,col="green")
